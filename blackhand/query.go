...
func fromJSON(root string, topic string, partition int32, input interface{}) (Query, error) {
		...
		if v, ok := mapped["tag"]; ok && v != nil {
			...
			if tk, ok := kv["key"]; ok && tk != nil {
				if tv, ok := kv["value"]; ok && tv != nil {
					...
					queries = append(queries, NewTermQuery(root, topic, partition, sk, sv))
					added = true
				}
			}
			if !added {
		...
		if v, ok := mapped["and"]; ok && v != nil {
			...
			if ok {
				...
				for _, subQuery := range list {
					q, err := fromJSON(root, topic, partition, subQuery)
					if err != nil {
						return nil, err
					}
					and.AddSubQuery(q)
